      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-display font-black text-neu-text flex items-center gap-3">
            <UsersIcon className="text-neu-accent w-8 h-8" /> Manajemen Pengguna
          </h1>
          <p className="text-neu-text mt-1 text-sm font-black">Atur pengguna sistem dan hak akses (role).</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 mb-6 bg-neu-bg p-1.5 rounded-xl w-fit border border-neu-bg">
        <button
          onClick={() => setActiveTab("users")}
          className={`px-6 py-2 rounded-lg font-black text-sm transition-all flex items-center gap-2 ${activeTab === "users" ? "bg-white shadow-sm text-neu-accent" : "text-neu-text hover:text-neu-accent"}`}
        >
          <UsersIcon className="w-4 h-4" /> Pengguna
        </button>
        <button
          onClick={() => setActiveTab("roles")}
          className={`px-6 py-2 rounded-lg font-black text-sm transition-all flex items-center gap-2 ${activeTab === "roles" ? "bg-white shadow-sm text-neu-accent" : "text-neu-text hover:text-neu-accent"}`}
        >
          ...
          {activeTab === "users" ? (
            ...
            <Card className="border-primary/20 shadow-lg shadow-primary/5 sticky top-24">
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Username</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Nama Lengkap</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Role</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Password {editingUserId && "(Kosongkan jika tidak diubah)"}</label>
                ...
                {!editingUserId && (
                  <div className="space-y-1.5">
                    <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Konfirmasi Password</label>
            ...
            <Card className="border-primary/20 shadow-lg shadow-primary/5 sticky top-24">
              ...
              <CardContent className="pt-6 space-y-6">
                <div className="space-y-1.5">
                  <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Nama Role</label>
                  <input 
                    value={roleForm.name} 
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between px-1">
                    <label className="text-xs font-black uppercase text-neu-text tracking-widest pl-1">Hak Akses / Permissions</label>
                    <div className="flex gap-2">
                       <button onClick={() => handleAllPermissionsToggle(true)} className="text-xs font-black uppercase text-primary hover:text-primary/70 transition-colors flex items-center gap-1">
                         <CheckCheck className="w-3 h-3" /> Full Access
                       </button>
                       <button onClick={() => handleAllPermissionsToggle(false)} className="text-xs font-black uppercase text-rose-500 hover:text-rose-500/70 transition-colors flex items-center gap-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {PERMISSION_ACTIONS.map(action => (
                            <label key={`${menu}-${action}`} className="flex items-center gap-1.5 cursor-pointer group">
                              ...
                              />
                              <span className="text-xs font-black text-neu-text group-hover:text-neu-accent uppercase">{action}</span>
                            </label>
                          ))}
                        </div>
                      </div>
             ...
             <Card className="border-border/50 shadow-xl shadow-black/5 overflow-hidden">
               <CardHeader className="border-b border-border/50 bg-secondary/10 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                 ...
                 <div className="relative w-full md:w-64">
                   <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neu-text" />
                   <input 
                     value={userSearch}
               ...
               <CardContent className="p-0 overflow-x-auto">
                 <table className="w-full text-sm text-left border-collapse">
                    <thead className="text-xs font-black text-neu-text uppercase bg-neu-bg/50 border-b border-neu-bg">
                      <tr>
                        <th className="px-4 py-3 font-black tracking-widest uppercase">Username</th>
                        <th className="px-4 py-3 font-black tracking-widest uppercase">Nama Lengkap</th>
                        <th className="px-4 py-3 font-black tracking-widest uppercase">Role</th>
                   ...
                   <tbody className="divide-y divide-border/30">
                     {isLoadingUsers && <tr><td colSpan={4} className="text-center py-20"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary mx-auto"></div></td></tr>}
                      {!isLoadingUsers && filteredUsers?.length === 0 && (
                        <tr><td colSpan={4} className="text-center py-20 text-neu-text font-black uppercase">Pengguna tidak ditemukan.</td></tr>
                      )}
                      {filteredUsers?.map(user => (
                        <tr key={user.id} className={cn("hover:bg-primary/[0.02] transition-colors", me?.id === user.id && "bg-primary/[0.03]")}>
                          <td className="px-4 py-3 font-black text-neu-text flex items-center gap-2">
                             {user.username}
                          ...
                          <td className="px-4 py-3 font-black text-neu-text">{user.name}</td>
                          <td className="px-4 py-3">
                            <span className="bg-primary/10 text-primary border border-primary/20 px-2.5 py-1 rounded-lg text-xs font-black uppercase tracking-wider">
                              {user.role}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {me?.id !== user.id ? (
                                ...
                                <button disabled title="You cannot delete yourself" className="p-2 text-neu-text/30 cursor-not-allowed rounded-xl border border-border/20"><Trash2 className="w-4 h-4" /></button>
                              )}
                            </div>
                          </td>
                        </tr>
            ...
            <Card className="border-border/50 shadow-xl shadow-black/5 overflow-hidden">
               ...
               <CardContent className="p-0 overflow-x-auto">
                 <table className="w-full text-sm text-left border-collapse">
                    <thead className="text-xs font-black text-neu-text uppercase bg-neu-bg/50 border-b border-neu-bg">
                      <tr>
                        <th className="px-4 py-3 font-black tracking-widest uppercase w-1/4">Nama Role</th>
                        <th className="px-4 py-3 font-black tracking-widest uppercase">Ringkasan Hak Akses</th>
                        <th className="px-4 py-3 text-center font-black tracking-widest uppercase w-[100px]">Aksi</th>
                      </tr>
                    </thead>
                   <tbody className="divide-y divide-border/30">
                      ...
                      {!isLoadingRoles && filteredRoles?.length === 0 && (
                        <tr><td colSpan={3} className="text-center py-20 text-neu-text font-black uppercase">Role tidak ditemukan.</td></tr>
                      )}
                      {filteredRoles?.map(role => (
                        <tr key={role.id} className="hover:bg-primary/[0.02] transition-colors align-top">
                          <td className="px-4 py-5">
                            <div className="font-black text-neu-text tracking-tight">{role.name}</div>
                            <div className="text-xs font-medium tracking-tight text-neu-text font-mono mt-1">ID: {role.id}</div>
                          </td>
                          <td className="px-4 py-5">
                            <div className="flex flex-wrap gap-2">
                              {role.permissions && Object.entries(role.permissions).map(([menu, actions]: [string, any]) => {
                                ...
                                return (
                                    <div key={menu} className="flex flex-col gap-1.5 border border-neu-accent/10 p-2 rounded-xl bg-neu-bg hover:border-neu-accent/30 transition-colors">
                                      <span className="text-xs font-black uppercase text-neu-accent tracking-widest">{menu}</span>
