import { db, penjualanTable, masterBarangTable, masterOnlineShopTable, customerTable } from "./index.js";
...
async function seedSamples() {
  ...
  try {
    // 1. Get some required master data
    ...
    if (products.length === 0 || shops.length === 0 || customers.length === 0) {
      console.error("❌ Need master data (products, shops, customers) in database to seed samples.");
      console.log("Please make sure Master Barang, Master Online Shop, and Customer are not empty.");
      return;
    }
    ...
    const samples = [
      {
        ...
        nilaiOnlineShop: String(Number(products[0].hargaJual) * 0.975), // minus 2.5% fee
        statusCair: "pending",
      ...
      },
      {
        ...
        nilaiOnlineShop: String(Number(products[2].hargaJual) * 0.98), // minus 2% fee
        statusCair: "pending",
